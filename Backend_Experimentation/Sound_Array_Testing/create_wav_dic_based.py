...
UNIT_LENGTH = 2
notes_to_play = 'E4E4F4G4G4F4E4D4C4C4D4E4E4D4D4**'
sampleRate = 44100.0
duration = int(input("Enter Duration"))
SAMPLE_LEN = sampleRate * duration
#frequency = note_frequency[note_list.index('a')]
notes_to_play_list = [notes_to_play[i:i+UNIT_LENGTH] for i in range(0, len(notes_to_play), UNIT_LENGTH)]

frequenies_to_play = []
for x in notes_to_play_list:
        frequenies_to_play.append(note_dic[x])
