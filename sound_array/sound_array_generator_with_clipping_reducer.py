...
def note_clipper(sampleRate, frequency, SAMPLE_LEN):
        """ Clips the note until it ends with 0"""
        note_product = []
        sounds = []
        for i in range(0, int(SAMPLE_LEN)):
                note_product.append(int(32767.0*math.sin(frequency*math.pi*float(i)/float(sampleRate))))
        while note_product[-1] != 0:
                note_product.pop()
        for product in note_product:
                packed_value = struct.pack('<h', product)
...
for note, frequency in note_list:
        ...
        sounds = note_clipper(sampleRate, frequency, SAMPLE_LEN) 
